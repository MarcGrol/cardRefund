...
import (
	...
	"github.com/Duxxie/platform/backend/lib/errorhandling"
...
func (ts *CardReturnService) HTTPHandlerWithRouter(router *mux.Router) *mux.Router {
	subRouter := router.PathPrefix("/api/cardrefund").Subrouter()

	subRouter.HandleFunc("/", askForCardRefund(ts)).Methods("POST")

	subRouter.HandleFunc("/{cardNumber}", getCardRefundDetails(ts)).Methods("GET")

	subRouter.HandleFunc("/{cardNumber}/qrcode", getCardRefundDetailsFromQrCode(ts)).Methods("GET")
